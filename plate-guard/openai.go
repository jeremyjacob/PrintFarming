...
This camera angle normally does not show every edge or the full perimeter of the plate. Cropped plate edges, perspective, and the printer's toolhead, gantry, rails, lead screws, rear chute, walls, or other fixed mechanism in the frame do not make the view unusable and are not objects on the plate. Set plate_visible=true when the normal camera view is clear enough that a retained printed part would be visible. Set plate_visible=false and is_empty=false only when darkness, severe blur, a foreign occlusion, or another image problem prevents detecting a retained part in the normal observation area.

The build plate is the broad, flat, dark textured plane across the lower portion of the image. Its texture, seams, edges, tabs, highlights, and broad dark or bright regions caused by shadows and glare are normal plate appearance, not printed parts. Do not call something a printed part merely because a tonal region looks raised. Set is_empty=false only when there is clear visual evidence of discrete freestanding geometry, loose material, or another physical object sitting on top of that plane. A retained print has visible object boundaries, height, and structure distinct from the flat plate surface. Normal surface texture, discoloration, shadows, reflections, and tiny harmless strands do not make the plate occupied. Never assume an ejection succeeded merely because this is a completion photo, but do not invent objects from lighting or fixed printer geometry. When no discrete object is actually visible on the build surface, set is_empty=true.`
