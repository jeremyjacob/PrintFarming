...
[`plate-guard/`](plate-guard/) contains a fail-closed Go service that receives Bambuddy print-completion webhooks, checks fresh build-plate images with `gpt-5.6-terra` and Bambuddy's calibrated local detector, and releases the next queued print only after every check passes.
